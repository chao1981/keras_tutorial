...
y_train = np.reshape(y_train, (int(y_train.shape[0]/time_steps), time_steps,x_train.shape[1]))

x_test = np.reshape(x_test, (int(x_test.shape[0]/time_steps), time_steps,x_test.shape[1]))
y_test = np.reshape(y_test, (int(y_test.shape[0]/time_steps), time_steps,y_test.shape[1]))
print(x_train.shape)
...
checkpoint = ModelCheckpoint('./weights/more_rnn_weights.h5',  # model filename
                             monitor='val_loss',  # quantity to monitor
                             verbose=1,  # verbosity - 0 or 1
                             save_best_only=True,  # The latest best model will not be overwritten
                             mode='min')  # The decision to overwrite model is m

model.load_weights("./weights/more_rnn_weights.h5")
# model.fit(x_train, y_train, batch_size=512, epochs=100, shuffle=True,validation_split=0.5, callbacks=[checkpoint],verbose=1)
