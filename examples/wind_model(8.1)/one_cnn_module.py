...
# x=Conv1D(64, 3, activation='relu')(input_shape)
# x=LSTM(64, return_sequences=True)(x)
# x=LSTM(32)(x)
# x=Dense(2, activation='linear')(x)
# model=Model(inputs=input_shape,outputs=x)
# model.summary()
# plot_model(model,"cnn_rnn.png",show_shapes=True)
...
checkpoint = ModelCheckpoint('./weights/cnn_weights.h5',  # model filename
                             monitor='val_loss', # quantity to monitor
                             verbose=1, # verbosity - 0 or 1
                             save_best_only= True, # The latest best model will not be overwritten
                             mode='min') # The decision to overwrite model is m
model.load_weights("./weights/cnn_weights.h5")
# model.fit(x_train, y_train, batch_size=512, epochs=100, validation_split=0.3, callbacks=[checkpoint],verbose=1)
...
plt.legend()
# plt.savefig('cnn_output_power0.png', bbox_inches='tight',dpi=300)
plt.show()
...
plt.legend()
# plt.savefig('cnn_output_power1.png', bbox_inches='tight',dpi=300)
plt.show()
